...
import { TrendingUp,Wallet,Zap,Shield,BarChart2,ExternalLink,RefreshCw,CheckCircle,Droplets,ChevronRight,Activity } from "lucide-react";
import { AreaChart,Area,XAxis,YAxis,Tooltip,ResponsiveContainer } from "recharts";

const BACKEND = "https://litvault-0q0n.onrender.com";
const APY_DATA=[{day:"Apr 15",apy:0},{day:"Apr 17",apy:4.2},{day:"Apr 19",apy:6.8},{day:"Apr 21",apy:5.9},{day:"Apr 23",apy:8.4},{day:"Apr 25",apy:7.1},{day:"Apr 27",apy:9.3},{day:"Today",apy:11.2}];
    ...
    <div className="glass rounded-3xl p-6 accent-border">
      ...
      {amount&&parseFloat(amount)>0&&(
        <div className="bg-[#0a0f16] rounded-xl p-3 mb-4 text-xs font-mono space-y-1">
          ...
          {tab==="deposit"&&<div className="flex justify-between text-vault-dim"><span>Est. APY</span><span className="text-vault-accent">~11.2%</span></div>}
        </div>
      )}
    ...
    <div className="min-h-screen">
      <header className="glass border-b border-vault-border sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="hidden sm:block text-vault-dim text-xs font-mono border border-vault-border rounded-full px-2 py-0.5">LiteForge Testnet</span>
          </div>
          <div>
            {address
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        <div className="text-center mb-12">
          ...
          <p className="text-vault-dim max-w-md mx-auto">
            ...
            <div className="glass rounded-3xl p-6">
              ...
              <ResponsiveContainer width="100%" height={180}>
                <AreaChart data={APY_DATA}>
                  ...
                  <Tooltip contentStyle={{background:"#0d1117",border:"1px solid #1a2332",borderRadius:8,fontSize:11}} formatter={v=>[`${v}%`,"APY"]}/>
          ...
          <div>
            {!address
              ?<div className="glass rounded-3xl p-8 accent-border text-center">
                ...
                <p className="text-vault-dim text-sm mb-2">Supports MetaMask, Trust Wallet, OKX, Coinbase Wallet</p>
                <p className="text-vault-dim text-xs mb-6 font-mono">Chain: LiteForge Testnet (4441)</p>
                <button onClick={connect} disabled={connecting}
              ...
              :<VaultPanel
                vaultStats={vaultStats} userInfo={userInfo} zkLTCBalance={zkLTCBalance}
        ...
        <footer className="mt-16 pt-6 border-t border-vault-border flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <Zap size={12} className="text-vault-accent"/>
            <span className="font-semibold text-sm shimmer-text">LitVault</span>
            <span className="text-vault-dim text-xs font-mono">· Built on LiteForge Testnet</span>
          </div>
          <div className="flex gap-4 text-xs font-mono text-vault-dim">
            <a href="https://liteforge.explorer.caldera.xyz" target="_blank" rel="noreferrer" className="hover:text-vault-accent flex items-center gap-1"><ExternalLink size={10}/> Explorer</a>
            <a href="https://github.com/arpdoul/litvault" target="_blank" rel="noreferrer" className="hover:text-vault-accent flex items-center gap-1"><ExternalLink size={10}/> GitHub</a>
            <a href="https://builders.litvm.com" target="_blank" rel="noreferrer" className="hover:text-vault-accent flex items-center gap-1"><ExternalLink size={10}/> Builders</a>
          </div>
        </footer>
      </div>
    </div>
